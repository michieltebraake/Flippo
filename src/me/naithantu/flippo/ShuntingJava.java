...
public class ShuntingJava {
	ShuntingJava() {}
	
	public double calcAnswer(List<String> in){
		String[] input = in.toArray(new String[0]);
	...
	public static Double infixToRPN(String[] inputTokens) {
		...
		for (String arg : out.toArray(output)) {
            if (arg.matches("[\\.0-9]+")) {
                calcStack.push(Double.parseDouble(arg));
            }
            else if (arg.matches("[*/+-]{1}")) {
	...
	public static double calcRPN(String args) {
